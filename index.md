## Michael Mogford

[LinkedIn](https://www.linkedin.com/in/michael-mogford-a10428136/)  [github](https://github.com/MLMogford)

I am a consultant machine learning engineer and data scientist working on a variety of client projects and developing MLOps systems that maintain machine learning algorithms in production.


***My academic and industry research and development experience includes:*** 
 - `multimodal fusion` of histochemical information with ultrasound image data for breast cancer molecular subtype classification
 - `development of a ranking loss function` to optimise sensitivity at a set specificity for state-population scale image classification for breast cancer screening
 - chief investigator in a pilot clinical drug trial to investigate the effects of long term opioid use for chronic pain on breathing stability and central nervous system inflammation



***Skills and abilities:*** 
 - Strong analytical skills 
 - Experimental design and hypothesis testing 
 - Algorithm development for ML systems 
 - Clear communication skills of complex concepts to a broad audience 
 - Current Senior First Aid Certificate  
 - Cross-cultural experience living in Beijing for five years 
    - Chinese language study at Peking University 
    - International relations research and event organisation (2015 Boyer Lecture) at the Peking University Australian Studies Centre 
    - CS masters at Tsinghua University 

My CV, outlining my broad experiences and qualifications can be found [here](cv). 

Read more about my [research](research). 

I also have some recommended material for anyone interested in [China](china-index). 

I am working on collecting some of my thoughts on [machine learning](ml-index), the tools available and my excperience with those tools.
